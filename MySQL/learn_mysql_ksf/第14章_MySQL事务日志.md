...
> 1. 一个事务 包含多条 SQL 语句。
>
> 例如，BEGIN; UPDATE ...; DELETE ...; COMMIT; 是一个事务。
>
> 2. 每条 SQL 语句 又会分解为多个底层的 mtr。
>
> 例如，UPDATE 语句可能需要修改表的主键索引、辅助索引等，分别是不同的 Mini-Transaction。
>
> 3. 每个 mtr 会记录自己的 redo log，用于崩溃恢复。
>
> 每个 mtr 操作的是数据库的一个底层数据页。
>
> 4. redo log 是最小的日志单元，它们记录了具体的修改内容，比如某一页的某个字节发生了变化。
