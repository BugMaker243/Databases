...
![image-20220329180747441](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329180747441.png)
...
![image-20220329184556306](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329184556306.png)
...
![image-20220329212518542](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329212518542.png)



最早的数据情况。

![image-20220329205328322](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329205328322.png)

下图 Pos  236 到1071 是张三林俊杰、张三3 生成的sql

![image-20220329211054299](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329211054299.png)

不小心改错了成这样了。

![image-20220329212708613](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329212708613.png)
...
![image-20220329213102535](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329213102535.png)

因为id 3只是修改。所以，后面又创建id3 就报错了。

![image-20220329213254839](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329213254839.png)
...
![image-20220329182236159](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182236159.png)
...
![image-20220329182304420](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182304420.png)

为了安全起见，可以设置为 `1` ，表示每次提交事务都会执行fsync，就如同 **redo log 刷盘流程** 一样。最后还有一种折中方式，可以设置为N(N>1)，表示每次提交事务都write，但累积N个事务后才fsync。

![image-20220329182335630](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182335630.png)
...
![image-20220329182419294](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182419294.png)
...
![image-20220329182435171](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182435171.png)

由于binlog没写完就异常，这时候binlog里面没有对应的修改记录。因此之后用binlog日志恢复数据时，就会少这一次更新，恢复出来的这一行c值是o，而原库因为redo log日志恢复，这一行c值是1，最终数据不一致。

![image-20220329182448251](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182448251.png)

为了解决两份日志之间的逻辑一致问题，InnoDB存储引擎使用**两阶段提交**方案。原理很简单，将redo log的写入拆成了两个步骤prepare和commit，这就是**两阶段提交**。

![image-20220329182513353](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182513353.png)

使用 **两阶段提交** 后，写入binlog时发生异常也不会有影响，因为MySQL根据redo log日志恢复数据时，发现redolog还处于prepare阶段，并且没有对应binlog日志，就会回滚该事务。

![image-20220329182540698](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182540698.png)

另一个场景，redo log设置commit阶段发生异常，那会不会回滚事务呢？

![image-20220329182604484](http://jason243.online/DatabasesNote/MySQL/learn_mysql_ksf/第17章_其它数据库日志.assets/image-20220329182604484.png)
